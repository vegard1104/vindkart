...
L.tileLayer('https://cache.kartverket.no/v1/wmts/1.0.0/topo/default/webmercator/{z}/{y}/{x}.png', {
    ...
    attribution: '&copy; <a href="https://kartverket.no">Kartverket</a> | V\u00e6r: Yr/MET Norway | NVE/Varsom',
...
// GPS Geolocation â replaces the red center dot
...
function drawOverlay() {
    ...
    const gs = GRID_SIZE, px = overlayCanvas.width*0.08, py = overlayCanvas.height*0.08;
...
// =====================================================================
// YR SYMBOL CODE TO WMO MAPPING
// =====================================================================
function yrSymbolToWmo(sym) {
    if (!sym) return 0;
    var s = sym.replace(/_day|_night|_polartwilight/g, '');
    var map = {
        ...
        'rainandthunder':95, 'sleetandthunder':95, 'snowandthunder':95,
        'lightrainandthunder':95, 'heavyrainandthunder':96,
        'lightrainshowersandthunder':95, 'rainshowersandthunder':95, 'heavyrainshowersandthunder':96,
        'lightsleetandthunder':95, 'heavysleetandthunder':96,
        'lightsnowandthunder':95, 'heavysnowandthunder':96,
    ...
    return map[s] !== undefined ? map[s] : 3;
}

function calcWindChill(tempC, windMs) {
    var wkmh = windMs * 3.6;
    if (tempC <= 10 && wkmh > 4.8) {
        return 13.12 + 0.6215*tempC - 11.37*Math.pow(wkmh,0.16) + 0.3965*tempC*Math.pow(wkmh,0.16);
    }
    return tempC;
}
...
async function fetchWeatherGrid() {
    ...
    statusEl.textContent = 'Henter v\u00e6rdata fra Yr...'; statusEl.classList.add('loading');
    ...
    try {
        ...
        for (let i=0;i<GRID_SIZE;i++) {
            for (let j=0;j<GRID_SIZE;j++) {
                const url = YR_API_BASE + '?lat=' + lats[i].toFixed(4) + '&lon=' + lngs[j].toFixed(4);
                fetches.push(
                    fetch(url).then(function(res) {
                        if (!res.ok) throw new Error('HTTP ' + res.status);
                        return res.json();
                    }).then(function(json) { return { i:i, j:j, data:json }; })
                    .catch(function(err) { console.warn('Yr fetch ['+i+']['+j+']:', err.message); return null; })
                );
            }
        }
        statusEl.textContent = 'Henter ' + fetches.length + ' v\u00e6rpunkter fra Yr...';
        const results = await Promise.all(fetches);
        const valid = results.filter(function(r) { return r !== null; });
        if (valid.length === 0) throw new Error('Ingen data mottatt fra Yr. Pr\u00f8v igjen.');
        const newDates = [];
        var firstTS = valid[0].data.properties.timeseries;
        for (var ti=0; ti<firstTS.length; ti++) {
            var utc = new Date(firstTS[ti].time);
            var oslo = new Date(utc.toLocaleString('en-US', { timeZone: 'Europe/Oslo' }));
            var dt = oslo.getFullYear()+'-'+String(oslo.getMonth()+1).padStart(2,'0')+'-'+String(oslo.getDate()).padStart(2,'0');
            if (!newDates.includes(dt) && newDates.length < FORECAST_DAYS) newDates.push(dt);
        }
        const newGrid = {};
        for (const ddt of newDates) { newGrid[ddt]={}; for (let h=0;h<24;h++) {
            newGrid[ddt][h] = {
                ...
                visibility: Array.from({length:GRID_SIZE},()=>new Array(GRID_SIZE).fill(10000)),
                dewpoint: Array.from({length:GRID_SIZE},()=>new Array(GRID_SIZE).fill(0)),
                pressure: Array.from({length:GRID_SIZE},()=>new Array(GRID_SIZE).fill(1013)),
        ...
        for (const r of valid) {
            var ts = r.data.properties.timeseries;
            for (var ti=0; ti<ts.length; ti++) {
                var entry = ts[ti];
                var utc = new Date(entry.time);
                var oslo = new Date(utc.toLocaleString('en-US', { timeZone: 'Europe/Oslo' }));
                var dtStr = oslo.getFullYear()+'-'+String(oslo.getMonth()+1).padStart(2,'0')+'-'+String(oslo.getDate()).padStart(2,'0');
                var hour = oslo.getHours();
                if (!newGrid[dtStr] || !newGrid[dtStr][hour]) continue;
                var cell = newGrid[dtStr][hour];
                var inst = entry.data.instant.details;
                cell.dirs[r.i][r.j] = inst.wind_from_direction || 0;
                cell.speeds[r.i][r.j] = inst.wind_speed || 0;
                cell.gusts[r.i][r.j] = inst.wind_speed ? inst.wind_speed * 1.4 : 0;
                cell.temps[r.i][r.j] = inst.air_temperature !== undefined ? inst.air_temperature : 0;
                cell.feelslike[r.i][r.j] = calcWindChill(inst.air_temperature || 0, inst.wind_speed || 0);
                cell.humidity[r.i][r.j] = inst.relative_humidity || 0;
                cell.cloudCover[r.i][r.j] = inst.cloud_area_fraction || 0;
                cell.dewpoint[r.i][r.j] = inst.dew_point_temperature || 0;
                cell.pressure[r.i][r.j] = inst.air_pressure_at_sea_level || 1013;
                var next1 = entry.data.next_1_hours;
                var next6 = entry.data.next_6_hours;
                var precipData = next1 || next6;
                if (precipData && precipData.details) {
                    cell.precip[r.i][r.j] = precipData.details.precipitation_amount || 0;
                }
                var symData = next1 || next6;
                if (symData && symData.summary) {
                    cell.weatherCode[r.i][r.j] = yrSymbolToWmo(symData.summary.symbol_code);
                }
            }
        }
        weatherGrid = newGrid;
        ...
        statusEl.textContent = 'Yr: ' + valid.length + '/' + (GRID_SIZE*GRID_SIZE) + ' punkter';
...
// MARKER SYSTEM â click to get detailed weather
...
async function fetchElevations() {
    ...
    // Use Yr/MET Norway elevation API
...
// TOPPTUR SYSTEM â dynamic from API (Overpass + Kartverket WFS)
...
// MAIN TOUR FETCH â combines all API sources
